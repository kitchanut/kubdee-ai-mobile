...
| E | **ความปลอดภัย** | release APK เซ็นด้วย **debug keystore สาธารณะ** — แอปที่มี accessibility + self-update ถูกปลอม APK ติดตั้งทับได้ |

**ถ้าทำได้แค่ 3 อย่างในเดือนนี้:** (1) เปลี่ยน release keystore [E] ก่อน user base โต, (2) วาง CI + sync-guard + version single-source [B,C], (3) เพิ่ม Sentry [A]
...
| # | Finding | หลักฐาน | Fix |
|---|---------|---------|-----|
| **H-1** [P] | **Release APK เซ็นด้วย debug keystore สาธารณะ** — แอปมี accessibility service คุม Shopee + อ่าน clipboard + self-update ดาวน์โหลด APK → ใครก็ build APK ปลอมเซ็น key เดียวกันติดตั้งทับเป็น "อัปเดต" ได้ | `android/app/build.gradle:110-115` release ใช้ `signingConfigs.debug` (pw `android`) | สร้าง release keystore จริง เก็บใน GitHub Secret (base64) อ่านผ่าน env. **ระวัง:** เปลี่ยน signature = ผู้ใช้เดิมต้อง uninstall/reinstall (ข้อมูล SQLite หาย) → **ทำเร็วสุดตอน user ยังน้อย** |
...
### Phase 1 — ความปลอดภัย + observability (1–2 วัน · Effort S-M · ต้องตัดสินใจ H-1)
7. **Release keystore จริง** + วางแผน signature migration (แจ้งผู้ใช้ uninstall/reinstall ครั้งเดียว) **[H-1]** ⚠️ *ยิ่งช้า user ยิ่งเยอะ ยิ่งเจ็บ*
8. `release.yml` — build APK บน Actions on tag `v*` + inject keystore จาก Secrets **[H-1, M-1]**
...
|--------|---------------------|
| **H-1 เปลี่ยน keystore** | กระทบผู้ใช้เดิม **ทุกคน** ครั้งเดียว (ต้อง reinstall, ข้อมูล local หาย) — ยิ่งช้า user ยิ่งเยอะ ยิ่งเจ็บ ควรทำ**เร็วที่สุด** |
